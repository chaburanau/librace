//! Assetto Corsa Rally dashboard data provider.
...
pub const title = "Assetto Corsa Rally | librace";
...
pub fn connect(ctx: *Context, _: std.Io) !void {
    ctx.client = try acr.connect(std.heap.page_allocator);
}
...
pub fn connectErrorHint(_: *Context, err: anyerror, w: *std.Io.Writer) !void {
    ...
    switch (err) {
        ...
        else => try w.print("Enter a live stage before running the dashboard.\n", .{}),
    }
}
...
pub fn fillData(ctx: *Context, data: *dashboard.Data) void {
    ...
    // AC gear encoding: 0 = reverse, 1 = neutral, 2+ = forward. Shift to a familiar display
    // scale (reverse = -1, neutral = 0, 1st = 1, ...).
    ...
    // SteerAngle is normalized [-1, 1]; scale to an approximate wheel range for display.
