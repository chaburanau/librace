//! Assetto Corsa Competizione dashboard data provider.
...
pub const title = "Assetto Corsa Competizione | librace";
...
pub fn connect(ctx: *Context, io: std.Io) !void {
    ctx.client = try acc.connect(std.heap.page_allocator, io, .{});
}
...
pub fn fillData(ctx: *Context, data: *dashboard.Data) void {
    ...
    data.header_right = std.fmt.bufPrint(&ctx.header_right_buf, "fields={d} pkt={d} rain={s}", .{
        c.fieldCount(),
        c.livePhysicsPacketId(),
        g.rainIntensityValue().label(),
    }) catch "?";

    // Session metadata - wchar strings decoded into owned buffers.
    data.track = nonEmpty(c.getString(acc.keys.static.track, &ctx.track_buf));
    data.car = nonEmpty(c.getString(acc.keys.static.car_model, &ctx.car_buf));
    ...
    // ACC gear encoding matches AC: 0 = reverse, 1 = neutral, 2+ = forward.
    data.gear = @as(f64, @floatFromInt(p.gear)) - 1;
    data.rpm = @floatFromInt(p.rpm);
    ...
    data.discovery_hint = std.fmt.bufPrint(&ctx.discovery_buf, "fields={d} cars={d} grip={s}", .{
        c.fieldCount(),
        g.active_cars,
        g.trackGripStatusValue().label(),
    }) catch "?";
}
...
fn formatDriver(ctx: *Context, c: *const acc.Client) []const u8 {
    const first = c.getString(acc.keys.static.player_name, &ctx.name_buf) orelse "";
    const last = c.getString(acc.keys.static.player_surname, &ctx.surname_buf) orelse "";
    if (first.len == 0 and last.len == 0) return "?";
    return std.fmt.bufPrint(&ctx.driver_buf, "{s} {s}", .{ first, last }) catch "?";
}

fn formatTrackLength(ctx: *Context, c: *const acc.Client) []const u8 {
