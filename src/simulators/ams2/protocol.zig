//! Automobilista 2 (`$pcars2$`) shared-memory layout.
//!
//! Reference: game install `Support/SharedMemory/AMS2_SharedMemoryExampleApp/SharedMemory.h`
//! (SHARED_MEMORY_VERSION = 14), also mirrored by community clients such as CREST2-AMS2.
//!
//! Layout notes:
//! - Named mapping `"$pcars2$"` using natural MSVC/C ABI alignment (not packed).
//! - Enable **Options → System → Shared Memory → Project CARS 2** in AMS2.
//! - `sequence_number` is odd while the game writes; even values are safe to read.
//! - Strings are UTF-8 `char` buffers. Speeds are m/s; `rpm` is already revolutions/minute.
//! - Large participant arrays sit mid-structure; the client copies them on demand.
...
pub const GameState = enum(u32) {
    ...
    pub fn label(self: GameState) []const u8 {
        return switch (self) {
            ...
            .ingame_inmenu_time_ticking => "Menu",
            .ingame_restarting => "Restarting",
            .ingame_replay => "Replay",
            .front_end_replay => "FE Replay",
            _ => "?",
        };
    }
};
...
pub const PitMode = enum(u32) {
    ...
    pub fn label(self: PitMode) []const u8 {
        return switch (self) {
            ...
            .in_garage => "Garage",
            .driving_out_of_garage => "Leaving Garage",
            _ => "?",
        };
    }
};
...
/// Full `$pcars2$` mapping (SHARED_MEMORY_VERSION 14).
pub const Shared = extern struct {
    ...
    wheel_local_position_y: [tyre_max]f32 = @splat(0),
    suspension_travel: [tyre_max]f32 = @splat(0),
    suspension_velocity: [tyre_max]f32 = @splat(0),
    air_pressure: [tyre_max]f32 = @splat(0),
    engine_speed: f32 = 0,
    engine_torque: f32 = 0,
    wings: [2]f32 = @splat(0),
    hand_brake: f32 = 0,

    current_sector1_times: [stored_participants_max]f32 = @splat(-1),
    current_sector2_times: [stored_participants_max]f32 = @splat(-1),
    current_sector3_times: [stored_participants_max]f32 = @splat(-1),
    fastest_sector1_times: [stored_participants_max]f32 = @splat(-1),
    fastest_sector2_times: [stored_participants_max]f32 = @splat(-1),
    fastest_sector3_times: [stored_participants_max]f32 = @splat(-1),
    fastest_lap_times: [stored_participants_max]f32 = @splat(-1),
    last_lap_times: [stored_participants_max]f32 = @splat(-1),
    laps_invalidated: [stored_participants_max]bool = @splat(false),
    race_states: [stored_participants_max]u32 = @splat(0),
    pit_modes: [stored_participants_max]u32 = @splat(0),
    orientations: [stored_participants_max][vec_max]f32 = @splat(@splat(0)),
    speeds: [stored_participants_max]f32 = @splat(0),
    car_names: [stored_participants_max][string_length_max]u8 = @splat(@splat(0)),
    car_class_names: [stored_participants_max][string_length_max]u8 = @splat(@splat(0)),

    enforced_pit_stop_lap: i32 = -1,
    translated_track_location: [string_length_max]u8 = @splat(0),
    translated_track_variation: [string_length_max]u8 = @splat(0),
    brake_bias: f32 = -1,
    turbo_boost_pressure: f32 = -1,
    tyre_compound: [tyre_max][tyre_compound_name_length_max]u8 = @splat(@splat(0)),
    pit_schedules: [stored_participants_max]u32 = @splat(0),
    highest_flag_colours: [stored_participants_max]u32 = @splat(0),
    highest_flag_reasons: [stored_participants_max]u32 = @splat(0),
    nationalities: [stored_participants_max]u32 = @splat(0),
    snow_density: f32 = 0,

    session_duration: f32 = 0,
    session_additional_laps: i32 = 0,

    tyre_temp_left: [tyre_max]f32 = @splat(0),
    tyre_temp_center: [tyre_max]f32 = @splat(0),
    tyre_temp_right: [tyre_max]f32 = @splat(0),

    drs_state: u32 = 0,
    ride_height: [tyre_max]f32 = @splat(0),

    joy_pad0: u32 = 0,
    d_pad: u32 = 0,

    anti_lock_setting: i32 = -1,
    traction_control_setting: i32 = -1,

    ers_deployment_mode: i32 = 0,
    ers_auto_mode_enabled: bool = false,

    clutch_temp: f32 = -273.16,
    clutch_wear: f32 = 0,
    clutch_overheated: bool = false,
    clutch_slipping: bool = false,

    yellow_flag_state: i32 = -1,

    session_is_private: bool = false,
    launch_stage: i32 = -1,
    ...
    pub fn translatedTrackLocation(self: *const Shared) []const u8 {
        return strings.cString(&self.translated_track_location);
    }

    pub fn translatedTrackVariation(self: *const Shared) []const u8 {
        return strings.cString(&self.translated_track_variation);
    }
...
/// Start of the large per-participant timing/name block.
pub const participant_timing_offset: usize = @offsetOf(Shared, "current_sector1_times");
/// Player/session fields after the timing/name block.
pub const after_timing_offset: usize = @offsetOf(Shared, "enforced_pit_stop_lap");
/// Start of per-participant pit/flag/nationality arrays.
pub const participant_flags_offset: usize = @offsetOf(Shared, "pit_schedules");
/// Remaining AMS2 tail (snow density through launch stage).
pub const ams2_tail_offset: usize = @offsetOf(Shared, "snow_density");
...
pub fn readSequenceNumber(view: []const u8) ?u32 {
    const offset = @offsetOf(Shared, "sequence_number");
...
test "layout matches AMS2 SharedMemory.h v14 C ABI" {
    try std.testing.expectEqual(@as(usize, 100), participant_info_size);
    try std.testing.expectEqual(@as(usize, 20700), shared_size);
    ...
    try std.testing.expectEqual(@as(usize, 6712), @offsetOf(Shared, "lap_invalidated"));
    try std.testing.expectEqual(@as(usize, 6716), @offsetOf(Shared, "best_lap_time"));
    try std.testing.expectEqual(@as(usize, 6888), @offsetOf(Shared, "anti_lock_active"));
    try std.testing.expectEqual(@as(usize, 7320), @offsetOf(Shared, "sequence_number"));
    try std.testing.expectEqual(@as(usize, 7408), participant_timing_offset);
    try std.testing.expectEqual(@as(usize, 19248), after_timing_offset);
    try std.testing.expectEqual(@as(usize, 19548), participant_flags_offset);
    try std.testing.expectEqual(@as(usize, 20572), ams2_tail_offset);
    try std.testing.expectEqual(@as(usize, 20696), @offsetOf(Shared, "launch_stage"));
}
...
test "string and unit helpers" {
    ...
    @memcpy(s.car_name[0.."Porsche 911 GT3 R".len], "Porsche 911 GT3 R");
    s.speed = 50.0;
    s.gear = 3;
    ...
    try std.testing.expectEqualStrings("Porsche 911 GT3 R", s.carName());
...
test "version and sequence readers" {
    var bytes: [shared_size]u8 = undefined;
    @memset(&bytes, 0);
    std.mem.writeInt(u32, bytes[0..4], 14, .little);
    std.mem.writeInt(u32, bytes[@offsetOf(Shared, "game_state")..][0..4], 2, .little);
    std.mem.writeInt(u32, bytes[@offsetOf(Shared, "sequence_number")..][0..4], 42, .little);

    try std.testing.expectEqual(@as(u32, 14), readVersion(&bytes).?);
    try std.testing.expectEqual(@as(u32, 2), readGameState(&bytes).?);
    try std.testing.expectEqual(@as(u32, 42), readSequenceNumber(&bytes).?);
}
