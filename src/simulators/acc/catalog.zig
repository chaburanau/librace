//! Generic, name-based access over the fixed ACC physics/graphics/static pages.
...
/// arrays are skipped and remain available through typed struct access.
...
test "catalog discovers ACC-specific fields" {
    try std.testing.expect(field_count > 140);
    try std.testing.expect(find("speed_kmh") != null);
    try std.testing.expect(find("rpm") != null);
    try std.testing.expect(find("current_max_rpm") != null);
    try std.testing.expect(find("track_grip_status") != null);
    try std.testing.expect(find("rain_intensity_in_30min") != null);
    try std.testing.expect(find("dry_tyres_name") != null);
    try std.testing.expect(find("tyre_contact_point") == null);
    try std.testing.expect(find("car_coordinates") == null);
}
...
test "catalog field metadata matches the struct layout" {
    ...
    const rain = find("rain_intensity").?;
    try std.testing.expectEqual(Page.graphics, rain.page);
    try std.testing.expectEqual(FieldType.i32, rain.field_type);
...
test "decodeWString converts UTF-16LE buffers and ignores numerics" {
    var stat: protocol.Static = .{};
    const name = std.unicode.utf8ToUtf16LeStringLiteral("Ferrari 296 GT3");
    @memcpy(stat.car_model[0..name.len], name);
    const bytes = std.mem.asBytes(&stat);

    var out: [64]u8 = undefined;
    try std.testing.expectEqualStrings("Ferrari 296 GT3", decodeWString(find("car_model").?, bytes, &out).?);
...
test "name iterator visits every catalogued field" {
    ...
    while (it.next()) |name| {
        ...
        if (std.mem.eql(u8, name, "rain_intensity")) saw_rain = true;
    }
    try std.testing.expectEqual(field_count, count);
    try std.testing.expect(saw_speed and saw_rain);
}
