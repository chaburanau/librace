...
//! struct access plus generic name-based lookup over the physics/graphics/static catalog.
...
pub const Client = struct {
    ...
    /// True when ACC has an active session or the physics packet counter has begun advancing.
...
test "generic access over an owned snapshot" {
    ...
    phys.gear = 5;
    phys.rpm = 7300;
    gfx.rain_intensity = @intFromEnum(protocol.RainIntensity.light);
    ...
    try std.testing.expectEqual(@as(f64, 7300), client.getNumber("rpm").?);
    try std.testing.expectEqual(@as(f64, 2), client.getNumber("rain_intensity").?);
    try std.testing.expect(client.getNumber("does_not_exist") == null);

    var buf: [64]u8 = undefined;
    try std.testing.expectEqualStrings("Ferrari 296 GT3", client.getString("car_model", &buf).?);
    try std.testing.expectEqualStrings("Spa", client.getString("track", &buf).?);
    try std.testing.expect(client.getString("speed_kmh", &buf) == null);
    try std.testing.expect(client.getNumber("car_model") == null);

    const h = client.resolve("rpm").?;
    try std.testing.expectEqual(@as(f64, 7300), client.read(h).?);
}
