//! Generic, name-based access over the fixed AC Rally physics/graphics/static pages.
//!
//! Like AC Evo (and unlike iRacing's runtime variable catalog), AC Rally exposes fixed C
//! structs. This module reflects over them at comptime to build a flat, immutable catalog
//! keyed by each field's protocol name, powering generic lookups (`getNumber`/`getString`/
//! `getRaw`) and discovery (name iteration). Adding or removing a field in `protocol.zig`
//! automatically updates what is reachable here.
//!
//! AC Rally strings are `wchar_t` (UTF-16LE) `[N]u16` arrays, classified here as `wstring`;
//! decoding them requires a caller-supplied UTF-8 buffer (see `decodeWString`).
...
pub const FieldDescriptor = struct {
    ...
    /// 1 for numeric scalars; N for a numeric array (`wheel_slip` = 4) or wstring code-unit length.
    count: usize,
};
...
/// Catalog numeric scalars, 1-D numeric arrays, and `[N]u16` wstrings. Multi-dimensional
/// arrays are skipped — those are reached through typed struct access.
...
/// Returns null for wstring fields or when the field lies outside `page_bytes`.
...
/// Null for non-string fields or when the field lies outside `page_bytes`.
...
test "catalog discovers numeric, wstring, and static fields" {
    try std.testing.expect(field_count > 40);
    try std.testing.expect(find("speed_kmh") != null);
    try std.testing.expect(find("rpms") != null);
    try std.testing.expect(find("normalized_car_position") != null);
    ...
    // Multi-dimensional arrays stay excluded (reached via typed struct access).
...
test "decodeWString converts a UTF-16LE buffer and ignores numerics" {
    ...
    const name = std.unicode.utf8ToUtf16LeStringLiteral("Lancia Delta");
    @memcpy(stat.car_model[0..name.len], name);
    const bytes = std.mem.asBytes(&stat);

    var out: [64]u8 = undefined;
    try std.testing.expectEqualStrings("Lancia Delta", decodeWString(find("car_model").?, bytes, &out).?);
