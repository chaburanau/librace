...
//! `Local\\acpmf_static` (session/car metadata, written once on load). AC Rally reuses the
//! original AC1 `SPageFile*` struct layout with `wchar_t` (UTF-16LE) strings.
//!
//! Design: fixed C structs (like AC Evo, unlike iRacing's runtime variable catalog). The client
//! exposes typed struct access (`physics()`, `graphics()`, `static()`) plus generic name-based
//! lookup (`getNumber`/`getString`/`getRaw`/`resolve`) and discovery (`fieldNameIterator`) built
//! from a comptime reflection of the protocol structs. Optional `keys.zig` provides common names.
