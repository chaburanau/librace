//! Assetto Corsa Rally client: opens the three classic AC shared-memory pages and exposes typed
...
pub const Client = struct {
    ...
    /// Note: AC Rally leaves the graphics page mostly unpopulated — use [`isConnected`] for liveness.
...
test "typed snapshot access and wstring decode" {
    ...
    try std.testing.expectEqualStrings("Lancia 037", client.static().?.carModelUtf8(&buf).?);
}
