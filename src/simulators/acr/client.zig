//! Assetto Corsa Rally client: opens the three classic AC shared-memory pages and exposes
//! typed struct access plus generic name-based lookup over the physics/graphics/static catalog.
...
pub const Client = struct {
    ...
    ///
    /// Note: AC Rally (unlike AC1/ACC) leaves the graphics page mostly unpopulated — `status`
    /// reads `off` even while actively driving. Use [`isConnected`] for liveness instead; this
    /// is retained for parity and for titles that do fill it.
    ...
    /// `packetId` from the live physics mapping (not the local copy). It advances every
    /// simulation step while a stage is loaded, so it is AC Rally's reliable liveness signal.
    pub fn livePhysicsPacketId(self: *const Client) i32 {
        return protocol.readPacketId(self.phys_mem.view) orelse 0;
    }

    /// True when the physics page carries an active session. AC Rally populates physics fully
    /// but leaves the graphics `status` flag at zero, so liveness is keyed off the physics
    /// `packetId` (non-zero ⇒ a stage is loaded and telemetry is flowing).
    pub fn isConnected(self: *const Client) bool {
        return self.livePhysicsPacketId() != 0;
    }
    ...
    /// Decode a `wchar_t` string field by protocol name into `out` as UTF-8. The returned
    /// slice borrows `out`. Null when the name is unknown or not a string field.
...
test "generic access over an owned snapshot" {
    ...
    phys.gear = 5;
    gfx.normalized_car_position = 0.25;
    const car = std.unicode.utf8ToUtf16LeStringLiteral("Lancia 037");
    @memcpy(stat.car_model[0..car.len], car);
    const track = std.unicode.utf8ToUtf16LeStringLiteral("Col de Turini");
    ...
    try std.testing.expectApproxEqAbs(@as(f64, 0.25), client.getNumber("normalized_car_position").?, 0.001);
    try std.testing.expect(client.getNumber("does_not_exist") == null);

    var buf: [64]u8 = undefined;
    try std.testing.expectEqualStrings("Lancia 037", client.getString("car_model", &buf).?);
    try std.testing.expectEqualStrings("Col de Turini", client.getString("track", &buf).?);
